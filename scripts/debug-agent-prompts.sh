...
BIN="${REPO_ROOT}/target/debug/closeredge-core"
...
echo "[debug-agent-prompts] building closeredge-core (no-op if up-to-date) …" >&2
( cd "${REPO_ROOT}" && cargo build --manifest-path Cargo.toml --bin closeredge-core >&2 )
...
# ── Delegate to `closeredge-core agent dump-all` ──────────────────────────
