...
# ── Extract binaries ─────────────────────────────────────────────────────────
tar -xzf "$TMPDIR/tarballs/closeredge-core-${VERSION}-x86_64-unknown-linux-gnu.tar.gz" \
  -C "$TMPDIR/bins"
mv "$TMPDIR/bins/closeredge-core" "$TMPDIR/bins/closeredge-core-amd64"

tar -xzf "$TMPDIR/tarballs/closeredge-core-${VERSION}-aarch64-unknown-linux-gnu.tar.gz" \
  -C "$TMPDIR/bins"
mv "$TMPDIR/bins/closeredge-core" "$TMPDIR/bins/closeredge-core-arm64"

chmod +x "$TMPDIR/bins/closeredge-core-amd64" "$TMPDIR/bins/closeredge-core-arm64"

# ── Build .deb packages ─────────────────────────────────────────────────────
echo "[apt] Building .deb packages ..."
bash "$REPO_ROOT/packages/deb/build.sh" "$TMPDIR/bins/closeredge-core-amd64" "${VERSION}" amd64
bash "$REPO_ROOT/packages/deb/build.sh" "$TMPDIR/bins/closeredge-core-arm64" "${VERSION}" arm64
