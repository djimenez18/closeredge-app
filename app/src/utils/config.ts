...
const APP_ENV = (import.meta.env.VITE_CLOSEREDGE_APP_ENV as string | undefined)
  ?.trim()
  .toLowerCase();

const DEFAULT_BACKEND_URL =
  APP_ENV === 'staging' ? 'https://staging-api.closeredge.ai' : 'https://api.closeredge.ai';
 ...
 *
 * Override at build time via `VITE_CLOSEREDGE_CORE_RPC_URL`.
 */
export const CORE_RPC_URL =
  import.meta.env.VITE_CLOSEREDGE_CORE_RPC_URL || 'http://127.0.0.1:7788/rpc';

/** Matches core `CLOSEREDGE_TOOL_TIMEOUT_SECS` (default 120s, max 3600s). */
...
export const E2E_RESTART_APP_AS_RELOAD =
  import.meta.env.VITE_CLOSEREDGE_E2E_RESTART_APP_AS_RELOAD === 'true';
export const E2E_DEFAULT_CORE_MODE =
  (import.meta.env.VITE_CLOSEREDGE_E2E_DEFAULT_CORE_MODE as string | undefined) || '';
...
export const SKILLS_GITHUB_REPO =
  import.meta.env.VITE_SKILLS_GITHUB_REPO || 'closeredgeai/closeredge-skills';
 ...
 * `closeredge.config_resolve_api_url`. If that call fails or returns an empty
...
export const TELEGRAM_BOT_USERNAME =
  (import.meta.env.VITE_TELEGRAM_BOT_USERNAME as string | undefined) || 'closeredge_bot';
 ...
 *
 * Derived from `VITE_CLOSEREDGE_APP_ENV` (set by CI for production / staging
...
/**
 * Canonical Sentry release identifier: `closeredge@<version>[+<short_sha>]`.
 *
...
export const SENTRY_RELEASE = BUILD_SHA
  ? `closeredge@${APP_VERSION}+${BUILD_SHA}`
  : `closeredge@${APP_VERSION}`;

/**
 * Minimum **desktop app** semver required for OAuth deep-link completion (`closeredge://oauth/success`).
 *
