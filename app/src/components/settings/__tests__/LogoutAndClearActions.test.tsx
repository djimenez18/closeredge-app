...
describe('LogoutAndClearActions', () => {
  ...
  it('surfaces the core error message when clearAllAppData fails (Windows file-lock guidance)', async () => {
    ...
    mockClearAllAppData.mockRejectedValueOnce(
      new Error(
        'Failed to remove C:\\Users\\me\\.openhuman because it is locked by another CloserEdge AI window or process. Close all CloserEdge AI windows and try again.'
      )
    );
    renderActions();
    ...
    expect(
      await screen.findByText(/locked by another CloserEdge AI window or process/)
    ).toBeInTheDocument();
  });
