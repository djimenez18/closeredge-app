...
describe('transcribeCloud', () => {
  ...
  it('rewrites "unknown method" errors to an actionable restart hint', async () => {
    ...
    const blob = new Blob([new Uint8Array([1])], { type: 'audio/webm' });
    await expect(transcribeCloud(blob)).rejects.toThrow(/Restart the CloserEdge AI desktop app/i);
  });
...
describe('transcribeWithFactory', () => {
  ...
  it('rewrites stale-sidecar "unknown method" errors', async () => {
    ...
    const blob = new Blob([new Uint8Array([1])], { type: 'audio/webm' });
    await expect(transcribeWithFactory(blob)).rejects.toThrow(/Restart the CloserEdge AI desktop app/i);
  });
