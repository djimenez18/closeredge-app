...
//!
//! Resolves the CloserEdge AI data directory the same way the core does
//! (`~/.openhuman` or `OPENHUMAN_WORKSPACE` override) and hands it to
//! [`closeredge_core::core::logging::init_for_embedded`], which installs a
...
use closeredge_core::core::logging::{self, log_directory};
...
pub(crate) fn resolve_data_dir() -> PathBuf {
    ...
    closeredge_core::openhuman::config::default_root_openhuman_dir().unwrap_or_else(|err| {
        eprintln!(
...
mod tests {
    ...
    fn reveal_logs_folder_errors_when_uninitialized() {
        ...
        if closeredge_core::core::logging::log_directory().is_none() {
