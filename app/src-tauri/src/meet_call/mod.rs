...
//! position) independently of the CloserEdge AI main window. The existing
...
pub async fn meet_call_open_window<R: Runtime>(
    ...
    // and confuse the user with multiple "Meet — CloserEdge AI" windows in
    ...
    // sees a meet window. The main CloserEdge AI UI is the only surface.
    //
    ...
    let builder = WebviewWindowBuilder::new(&app, &label, WebviewUrl::External(parsed.clone()))
        ...
        // becomes the macOS key window, the main CloserEdge AI window is
