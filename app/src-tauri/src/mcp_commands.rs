...
//! - `mcp_resolve_binary_path` — locate the `closeredge-core` binary on disk.
...
pub struct McpBinaryInfo {
    /// Absolute path to the `closeredge-core` binary.
    pub path: String,
...
/// `target/debug/closeredge-core[.exe]`. Returns the full path to the binary
/// when found, or `None` if the tree is exhausted.
fn find_debug_binary_walking_up(start: &std::path::Path) -> Option<PathBuf> {
    #[cfg(target_os = "windows")]
    let bin_name = "closeredge-core.exe";
    #[cfg(not(target_os = "windows"))]
    let bin_name = "closeredge-core";
...
/// Resolve the absolute path to the `closeredge-core` binary.
///
/// In dev builds (`cfg!(debug_assertions)`) we:
/// 1. Check `OPENHUMAN_CORE_BINARY_PATH` env var first.
/// 2. Walk up from `current_exe()` looking for `target/debug/closeredge-core`.
///
/// In release builds the binary is a sibling of the shell executable:
/// - macOS: `../MacOS/closeredge-core` relative to the host exe.
...
fn resolve_binary_path() -> Result<PathBuf, String> {
    ...
    #[cfg(target_os = "macos")]
    let candidate = {
        // macOS .app: Contents/MacOS/<host> → Contents/MacOS/closeredge-core
        exe.parent()
    ...
    if !candidate.exists() {
        return Err(format!(
            "closeredge-core binary not found at expected path: {}",
            candidate.display()
        ));
    }
...
/// Tauri command — resolve the `closeredge-core` binary path and OS name.
///
...
mod tests {
    ...
    /// `target/debug/closeredge-core` by walking up from the test executable.
    ///
    /// We only assert the path *contains* `closeredge-core` — the binary may or
    ...
    #[test]
    fn binary_path_result_contains_closeredge_core() {
        match resolve_binary_path() {
            Ok(p) => {
                let s = p.display().to_string();
                assert!(
                    s.contains("closeredge-core"),
                    "resolved path should contain 'closeredge-core', got: {s}"
                );
            }
            Err(e) => {
    ...
    fn find_debug_binary_returns_none_for_empty_dir() {
        ...
        // of /tmp (or equivalent) will contain target/debug/closeredge-core.
