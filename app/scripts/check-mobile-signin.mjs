...
async function run() {
  ...
  if (
    /^the-real-password$|yourdomain\.com$/.test(TEST_PASSWORD) ||
    /yourdomain\.com$/.test(TEST_EMAIL)
  ) {
    console.error(
      'Looks like the placeholder TEST_EMAIL/TEST_PASSWORD are still set — use a real test account.'
    );
    return 2;
  }
  ...
  if (authErr || !auth?.session) {
    ...
    if (authErr?.message === 'Invalid login credentials') {
      console.log('    → wrong email/password, OR the user is not Auto-Confirmed. Create one in');
      console.log('      Supabase → Authentication → Users → Add user (tick Auto Confirm).');
    }
    return 1;
  }
...
// Use process.exitCode + natural exit (no process.exit mid-async) so Windows
// libuv doesn't assert on a closing handle.
run()
  .then(code => {
