...
case "$OS" in
  Linux)
    ...
    cargo tauri build -c "$TAURI_CONFIG_OVERRIDE" --debug --no-bundle --features e2e-test-support -- --bin CloserEdgeAI
    ;;
  Darwin)
    # macOS: build .app bundle (wdio.conf points at
    # src-tauri/target/debug/bundle/macos/CloserEdge AI.app).
    echo "Building for macOS (.app bundle)..."
    cargo tauri build -c "$TAURI_CONFIG_OVERRIDE" --bundles app --debug --features e2e-test-support -- --bin CloserEdgeAI
    ;;
  MINGW*|MSYS*|CYGWIN*|Windows_NT)
    # Windows: bare .exe at src-tauri/target/debug/CloserEdgeAI.exe.
