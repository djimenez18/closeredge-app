...
use closeredge_core::openhuman::agent::task_board::{TaskBoard, TaskBoardCard, TaskCardStatus};
use closeredge_core::openhuman::config::Config;
use closeredge_core::openhuman::embeddings::NoopEmbedding;
use closeredge_core::openhuman::memory::query::{
...
use closeredge_core::openhuman::memory::tools::{
    MemoryForgetTool, MemoryRecallTool, MemoryStoreTool,
};
use closeredge_core::openhuman::memory::tree_policy::TreePolicy;
use closeredge_core::openhuman::memory::tree_source;
...
use closeredge_core::openhuman::memory_queue::{
...
use closeredge_core::openhuman::memory_sources::readers::reader_for;
use closeredge_core::openhuman::memory_sources::registry;
use closeredge_core::openhuman::memory_sources::rpc as memory_sources_rpc;
use closeredge_core::openhuman::memory_sources::status::{source_status, FreshnessLabel};
use closeredge_core::openhuman::memory_sources::sync::sync_source;
use closeredge_core::openhuman::memory_sources::types::{
...
use closeredge_core::openhuman::memory_store::trees::types::{
...
use closeredge_core::openhuman::memory_sync::canonicalize::email_clean;
use closeredge_core::openhuman::memory_sync::composio;
use closeredge_core::openhuman::memory_sync::composio::providers::profile::{
...
use closeredge_core::openhuman::memory_sync::composio::providers::profile_md::{
...
use closeredge_core::openhuman::memory_sync::composio::providers::sync_state::{
    extract_item_id, DailyBudget, SyncState, DEFAULT_DAILY_REQUEST_LIMIT,
};
use closeredge_core::openhuman::memory_sync::composio::providers::user_scopes;
...
use closeredge_core::openhuman::memory_sync::sync_status::{
    rpc as memory_sync_status_rpc, schemas as memory_sync_status_schemas,
};
use closeredge_core::openhuman::memory_sync::traits::{
    SyncOutcome as PipelineSyncOutcome, SyncPipeline, SyncPipelineKind,
};
use closeredge_core::openhuman::memory_tools::tools::{MemoryToolsListTool, MemoryToolsPutTool};
use closeredge_core::openhuman::memory_tools::{
...
use closeredge_core::openhuman::memory_tree::score::embed::Embedder;
...
use closeredge_core::openhuman::memory_tree::score::signals::{
...
use closeredge_core::openhuman::memory_tree::score::store as score_store;
use closeredge_core::openhuman::memory_tree::score::{resolver, ScoringConfig};
use closeredge_core::openhuman::memory_tree::summarise::{
    fallback_summary, SummaryContext, SummaryInput,
};
use closeredge_core::openhuman::memory_tree::tree::bucket_seal::LeafRef;
use closeredge_core::openhuman::memory_tree::tree_runtime::store as tree_runtime_store;
...
use closeredge_core::openhuman::memory_tree::{retrieval, score::embed};
...
use closeredge_core::openhuman::threads::title::{
...
use closeredge_core::openhuman::threads::ThreadsError;
use closeredge_core::openhuman::threads::{
    all_threads_controller_schemas, all_threads_registered_controllers,
};
use closeredge_core::openhuman::tools::traits::{PermissionLevel, Tool, ToolCategory};
    ...
    let extract_again = memory
        .extract_graph(
            ...
            &MemoryIngestionConfig {
                extraction_mode: closeredge_core::openhuman::memory::ExtractionMode::Chunk,
                ..Default::default()
            },
        )
        .await
...
async fn memory_source_readers_validate_and_use_local_inputs_only() {
    ...
    github.url = Some("https://github.com/closeredgeai/closeredge/tree/main".into());
    assert!(github_reader
...
async fn memory_thread_tree_and_sync_controller_schemas_execute_public_handlers() {
    ...
    assert_eq!(
        closeredge_core::openhuman::threads::schemas::schemas("missing").function,
        "unknown"
    );
    for function in [
        "list",
...
fn memory_schema_registries_and_query_tool_metadata_cover_public_surfaces() {
    ...
    for function in [
        ...
        let schema = closeredge_core::openhuman::memory::schemas::schemas(function);
    ...
    assert_eq!(
        closeredge_core::openhuman::memory::schemas::schemas("missing").function,
        "unknown"
    );

    let legacy_tree_schemas = closeredge_core::openhuman::memory::schema::all_controller_schemas();
    let legacy_tree_controllers =
        closeredge_core::openhuman::memory::schema::all_registered_controllers();
    ...
    for function in [
        ...
        let schema = closeredge_core::openhuman::memory::schema::schemas(function);
    ...
    assert_eq!(
        closeredge_core::openhuman::memory::schema::schemas("missing").function,
        "unknown"
    );
...
fn memory_tree_policy_and_source_registry_write_metadata_mirror() {
    ...
    let stats = closeredge_core::openhuman::memory_store::trees::types::EntityIndexStats {
        mention_count_30d: 9,
        distinct_sources: 4,
...
fn memory_tree_scoring_signal_helpers_cover_boundaries_and_serialization() {
    ...
    let regex_entities = closeredge_core::openhuman::memory_tree::score::extract::regex::extract(
...
fn memory_tree_runtime_store_buffers_and_retrieval_wire_helpers() {
    ...
    let source_factory = closeredge_core::openhuman::memory_tree::tree::TreeFactory::source(
        "gmail:alice@example.com|bob@example.com",
    );
    assert_eq!(
        source_factory.profile(),
        closeredge_core::openhuman::memory_tree::tree::TreeProfile::Source
    );
    assert_eq!(
    ...
    assert!(
        closeredge_core::openhuman::memory_tree::tree::new_tree_id(TreeKind::Global)
            .starts_with("global:")
    );
    assert!(closeredge_core::openhuman::memory_tree::tree::new_summary_id(2).contains(":L2-"));
    assert!(
        closeredge_core::openhuman::memory_tree::tree::registry::is_unique_violation(
    ...
    assert_eq!(
        closeredge_core::openhuman::memory_tree::tree::store::get_tree_by_scope(
            &config,
            TreeKind::Source,
            "gmail:alice@example.com|bob@example.com"
        )
...
async fn memory_read_rpc_score_index_and_summary_helpers_cover_dashboard_paths() {
    ...
    let empty =
        closeredge_core::openhuman::memory_tree::summarise::summarise(&config, &[], &empty_ctx)
            .await
            .expect("empty summarise avoids provider");
    assert_eq!(empty.token_count, 0);

    let embedder =
        closeredge_core::openhuman::memory_tree::score::embed::factory::build_embedder_from_config(
            &config,
        )
...
async fn memory_tools_and_user_scope_prefs_cover_public_execution_paths() {
    ...
    let scoped_client: closeredge_core::openhuman::memory_store::MemoryClientRef =
...
fn memory_tree_io_contract_types_round_trip_leaf_read_and_write_shapes() {
    ...
    let payload = closeredge_core::openhuman::memory_tree::io::TreeLeafPayload {
    ...
    let round_trip =
        closeredge_core::openhuman::memory_tree::io::TreeLeafPayload::from(leaf_ref.clone());
    ...
    let write_default_json = serde_json::to_value(
        closeredge_core::openhuman::memory_tree::io::TreeWriteRequest {
    ...
    let decoded_write: closeredge_core::openhuman::memory_tree::io::TreeWriteRequest =
    ...
    assert_eq!(
        ...
        closeredge_core::openhuman::memory_tree::io::TreeLabelStrategy::Empty
    );
    assert!(decoded_write.leaf.entities.is_empty());
    assert!(decoded_write.deferred);

    let outcome = closeredge_core::openhuman::memory_tree::io::TreeWriteOutcome {
    ...
    let read_request: closeredge_core::openhuman::memory_tree::io::TreeReadRequest =
        serde_json::from_value(json!({
            "tree_id": "tree-contract",
            "max_depth": 2,
            "query": "coverage",
    ...
    let hit = closeredge_core::openhuman::memory_tree::io::TreeReadHit {
    ...
    let result = closeredge_core::openhuman::memory_tree::io::TreeReadResult {
        hits: vec![hit],
        total: 4,
        tree_id: "tree-contract".into(),
    };
    ...
    let empty = closeredge_core::openhuman::memory_tree::io::TreeReadResult::empty(&tree);
...
fn gmail_post_processor_and_provider_registry_cover_public_edges() {
    let gmail_provider =
        closeredge_core::openhuman::memory_sync::composio::providers::gmail::GmailProvider::new();
...
fn memory_sync_profile_markdown_and_status_helpers_are_idempotent() {
    ...
    let now = 1_700_000_000_000_i64;
    assert_eq!(
        closeredge_core::openhuman::memory_sync::sync_status::types::FreshnessLabel::from_age_ms(
            Some(now - 30_000),
            now
        ),
        closeredge_core::openhuman::memory_sync::sync_status::types::FreshnessLabel::Active
    );
    assert_eq!(
        closeredge_core::openhuman::memory_sync::sync_status::types::FreshnessLabel::from_age_ms(
            Some(now - 30_001),
        ...
        closeredge_core::openhuman::memory_sync::sync_status::types::FreshnessLabel::Recent
    );
    assert_eq!(
        closeredge_core::openhuman::memory_sync::sync_status::types::FreshnessLabel::from_age_ms(
            None, now
        ),
        closeredge_core::openhuman::memory_sync::sync_status::types::FreshnessLabel::Idle
    );
}
...
async fn memory_sources_registry_rpc_and_schema_handlers_cover_crud_edges() {
    ...
    assert_eq!(
        closeredge_core::openhuman::memory_sources::schemas::schemas("read_item").function,
        "read_item"
    );
...
async fn memory_ops_public_handlers_cover_document_file_kv_graph_and_envelopes() {
    ...
    let init = closeredge_core::openhuman::memory::ops::memory_init(MemoryInitRequest {
    ...
    let sync_channel = closeredge_core::openhuman::memory::ops::memory_sync_channel(
        closeredge_core::openhuman::memory::ops::SyncChannelParams {
    ...
    let sync_all = closeredge_core::openhuman::memory::ops::memory_sync_all()
        .await
        .expect("sync all request")
        .value;
    assert!(sync_all.requested);
    let ingestion = closeredge_core::openhuman::memory::ops::memory_ingestion_status()
        .await
        .expect("ingestion status")
        .value;
    assert_eq!(ingestion.queue_depth, 0);
    let learn_none = closeredge_core::openhuman::memory::ops::memory_learn_all(
        closeredge_core::openhuman::memory::ops::LearnAllParams {
    ...
    let write =
        closeredge_core::openhuman::memory::ops::ai_write_memory_file(WriteMemoryFileRequest {
    ...
    let root_files =
        closeredge_core::openhuman::memory::ops::ai_list_memory_files(ListMemoryFilesRequest {
            relative_dir: "".into(),
        })
        .await
    ...
    let document_id = closeredge_core::openhuman::memory::ops::doc_put(
        closeredge_core::openhuman::memory::ops::PutDocParams {
    ...
    let namespaces = closeredge_core::openhuman::memory::ops::namespace_list()
        .await
        .expect("namespace list")
        .value;
    assert!(namespaces.iter().any(|candidate| candidate == namespace));
    let learn_disabled = closeredge_core::openhuman::memory::ops::memory_learn_all(
        closeredge_core::openhuman::memory::ops::LearnAllParams {
    ...
    let direct_docs = closeredge_core::openhuman::memory::ops::doc_list(Some(
        closeredge_core::openhuman::memory::ops::NamespaceOnlyParams {
            namespace: namespace.into(),
        },
    ))
    .await
    ...
    let envelope_docs =
        closeredge_core::openhuman::memory::ops::memory_list_documents(ListDocumentsRequest {
    ...
    let query = closeredge_core::openhuman::memory::ops::context_query(
        closeredge_core::openhuman::memory::ops::QueryNamespaceParams {
            namespace: namespace.into(),
    ...
    let recalled = closeredge_core::openhuman::memory::ops::context_recall(
        closeredge_core::openhuman::memory::ops::RecallNamespaceParams {
            namespace: namespace.into(),
            limit: Some(5),
        },
    )
    ...
    closeredge_core::openhuman::memory::ops::kv_set(
        closeredge_core::openhuman::memory::ops::KvSetParams {
            namespace: Some(namespace.into()),
            key: "state".into(),
    ...
    .await
    .expect("kv set");
    let kv = closeredge_core::openhuman::memory::ops::kv_get(
        closeredge_core::openhuman::memory::ops::KvGetDeleteParams {
            namespace: Some(namespace.into()),
            key: "state".into(),
        },
    )
    ...
    let kv_rows = closeredge_core::openhuman::memory::ops::kv_list_namespace(
        closeredge_core::openhuman::memory::ops::NamespaceOnlyParams {
            namespace: namespace.into(),
        },
    )
    ...
    assert!(
        closeredge_core::openhuman::memory::ops::kv_delete(
            closeredge_core::openhuman::memory::ops::KvGetDeleteParams {
                namespace: Some(namespace.into()),
                key: "state".into(),
            },
    ...
    closeredge_core::openhuman::memory::ops::graph_upsert(
        closeredge_core::openhuman::memory::ops::GraphUpsertParams {
            namespace: Some(namespace.into()),
            subject: "Alice".into(),
            predicate: "OWNS".into(),
    ...
    .await
    .expect("graph upsert");
    let relations = closeredge_core::openhuman::memory::ops::graph_query(
        closeredge_core::openhuman::memory::ops::GraphQueryParams {
            namespace: Some(namespace.into()),
    ...
    let tool_rule = closeredge_core::openhuman::memory::ops::tool_rule_put(
        closeredge_core::openhuman::memory::ops::ToolRulePutParams {
    ...
    let fetched_rule = closeredge_core::openhuman::memory::ops::tool_rule_get(
        closeredge_core::openhuman::memory::ops::ToolRuleRefParams {
            tool_name: "shell".into(),
            id: "ops-rule-1".into(),
        },
    )
    ...
    let listed_rules = closeredge_core::openhuman::memory::ops::tool_rule_list(
        closeredge_core::openhuman::memory::ops::ToolRuleListParams {
            tool_name: "shell".into(),
        },
    )
    .await
    ...
    let prompt_rules = closeredge_core::openhuman::memory::ops::tool_rules_for_prompt(
        closeredge_core::openhuman::memory::ops::ToolRulesForPromptParams {
    ...
    let tool_rules_json = closeredge_core::openhuman::memory::ops::tool_rules_json(
        closeredge_core::openhuman::memory::ops::ToolRuleListParams {
            tool_name: "shell".into(),
        },
    )
    ...
    assert!(
        closeredge_core::openhuman::memory::ops::tool_rule_delete(
            closeredge_core::openhuman::memory::ops::ToolRuleRefParams {
                tool_name: "shell".into(),
                id: "ops-rule-1".into(),
            },
    ...
    assert!(closeredge_core::openhuman::memory::ops::tool_rule_get(
        closeredge_core::openhuman::memory::ops::ToolRuleRefParams {
            tool_name: "shell".into(),
            id: "ops-rule-1".into(),
        },
    ...
    let delete_missing =
        closeredge_core::openhuman::memory::ops::memory_delete_document(DeleteDocumentRequest {
            namespace: namespace.into(),
    ...
    let deleted = closeredge_core::openhuman::memory::ops::doc_delete(
        closeredge_core::openhuman::memory::ops::DeleteDocParams {
            namespace: namespace.into(),
            document_id,
        },
    )
    ...
    let cleared = closeredge_core::openhuman::memory::ops::clear_namespace(
        closeredge_core::openhuman::memory::ops::ClearNamespaceParams {
            namespace: namespace.into(),
        },
    )
    .await
...
async fn memory_tree_retrieval_rpc_and_schema_wrappers_cover_empty_and_invalid_paths() {
    ...
    let schemas =
        closeredge_core::openhuman::memory_tree::retrieval::schemas::all_controller_schemas();
    let controllers =
        closeredge_core::openhuman::memory_tree::retrieval::schemas::all_registered_controllers();
    assert_eq!(schemas.len(), 4);
    assert_eq!(schemas.len(), controllers.len());
    assert_eq!(
        closeredge_core::openhuman::memory_tree::retrieval::schemas::schemas("missing").function,
        "unknown"
    );
    assert!(schemas
        .iter()
    ...
    let source = closeredge_core::openhuman::memory_tree::retrieval::rpc::query_source_rpc(
        &config,
        closeredge_core::openhuman::memory_tree::retrieval::rpc::QuerySourceRequest {
    ...
    assert!(
        closeredge_core::openhuman::memory_tree::retrieval::rpc::query_source_rpc(
            &config,
            closeredge_core::openhuman::memory_tree::retrieval::rpc::QuerySourceRequest {
                source_id: None,
    ...
    let search = closeredge_core::openhuman::memory_tree::retrieval::rpc::search_entities_rpc(
        &config,
        closeredge_core::openhuman::memory_tree::retrieval::rpc::SearchEntitiesRequest {
            query: "alice".into(),
    ...
    assert!(
        closeredge_core::openhuman::memory_tree::retrieval::rpc::search_entities_rpc(
            &config,
            closeredge_core::openhuman::memory_tree::retrieval::rpc::SearchEntitiesRequest {
                query: "alice".into(),
    ...
    let drill = closeredge_core::openhuman::memory_tree::retrieval::rpc::drill_down_rpc(
        &config,
        closeredge_core::openhuman::memory_tree::retrieval::rpc::DrillDownRequest {
    ...
    let fetch = closeredge_core::openhuman::memory_tree::retrieval::rpc::fetch_leaves_rpc(
        &config,
        closeredge_core::openhuman::memory_tree::retrieval::rpc::FetchLeavesRequest {
...
async fn memory_query_backend_and_tree_flush_wrappers_cover_public_edges() {
    ...
    let no_stale =
        closeredge_core::openhuman::memory_tree::tree::flush::flush_stale_buffers_default(
            &config,
            &closeredge_core::openhuman::memory_tree::tree::LabelStrategy::Empty,
        )
        .await
        .expect("flush empty buffers");
    assert_eq!(no_stale, 0);
    let missing_flush = closeredge_core::openhuman::memory_tree::tree::flush::force_flush_tree(
        &config,
        "tree:missing",
        None,
        &closeredge_core::openhuman::memory_tree::tree::LabelStrategy::Empty,
...
async fn tree_summarizer_ops_cover_validation_query_and_local_provider_guards() {
    ...
    let query = closeredge_core::openhuman::memory_tree::tree_runtime::ops::tree_summarizer_query(
        &config, "ops_ns", None,
    )
    .await
    .expect("query root");
    ...
    let provider_guard =
        closeredge_core::openhuman::memory_tree::tree_runtime::ops::tree_summarizer_run(
            &config, "ops_ns",
        )
        .await
        .unwrap_err();
    ...
    let rebuild_guard =
        closeredge_core::openhuman::memory_tree::tree_runtime::ops::tree_summarizer_rebuild(
            &config, "ops_ns",
        )
        .await
        .unwrap_err();
...
async fn memory_sources_types_registry_and_sync_state_cover_public_persistence_edges() {
    ...
    closeredge_core::openhuman::memory_sources::reconcile::ensure_composio_sources().await;
    ...
    entry.url = Some("https://github.com/closeredgeai/closeredge".into());
    ...
    let patch: registry::MemorySourcePatch = serde_json::from_value(json!({
        ...
        "path": "/tmp/repo",
        "glob": "**/*.md",
        "url": "https://github.com/closeredgeai/closeredge-skills",
        "branch": "main",
    ...
    assert_eq!(
        ...
        Some("https://github.com/closeredgeai/closeredge-skills")
    );
...
fn welcome_migration_public_entrypoint_covers_empty_marker_and_transcript_paths() {
    ...
    let result = closeredge_core::openhuman::threads::migrate_welcome_agent_artifacts(workspace)
    ...
    let second = closeredge_core::openhuman::threads::migrate_welcome_agent_artifacts(workspace)
